...
import static de.bernd_michaely.chiffres.fx.display.ScaleBox.SpaceGainingMode.*;
import static java.lang.Math.max;
import static java.util.Objects.requireNonNull;

/**
 * Component to show an encapsulated component scaled down or scrolled when
 * resized below its min size.
 *
 * @author Bernd Michaely
 */
public class ScaleBox implements DisplayComponent
{
	...
	public ScaleBox(Region region, boolean initializeImmediately,
		boolean upscaling, SpaceGainingMode spaceGainingMode)
	{
		this.region = requireNonNull(region, "Region is null");
		this.upscaling = upscaling;
		this.spaceGainingMode = (spaceGainingMode != null) ? spaceGainingMode : SCALING;
		this.root = isScalingMode() ? new Pane(region) : new ScrollPane(region);
	...
	{
		return SCALING.equals(this.spaceGainingMode);
	}
